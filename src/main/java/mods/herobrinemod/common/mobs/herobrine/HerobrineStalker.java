...
public class HerobrineStalker {
    ...
    public void spreadVirus() {
        ...
        for (EntityLivingBase entity : entities) {
            ...
            if (Config.mappings.containsKey(entity.getClass())) {
                try {
                    enemy = (EntityLivingBase) Config.mappings.get(entity.getClass())
                        .newInstance(world);
    ...
    private static void sendGlobalPhrase() {
        int message = MinecraftServer.getServer()
            .getEntityWorld().rand.nextInt(phrases.length);
        MinecraftServer.getServer()
            .getConfigurationManager()
            .sendChatMsg(new ChatComponentText(phrases[message]));
    }
    ...
    public void spawnFireRingOnPlayer(EntityPlayer player, int radius) {
        if (!Config.placeFire || player == null) {
            return;
        }
    ...
    public void RingOfEnemies(EntityPlayer player, int radius, int enemy) {
        if (!Config.animal_circle || player == null) {
            return;
        }
        World world = player.worldObj;
    ...
    public static String[] phrases = { "140%!", "E", "EEEEEEEEEEEEEE!", "Hi GameChap!", "Hi Berty!",
        "?!!!erehw si hctoN", "nevaeh ot yawriats", "!lleh ot emoclew", "!kcik esuohdnouR", "!enirboreH ma I", "Hi!",
        "?tey deracs", "ten.tfarcenimtenalp no erog tsoP", "!deirram si hctoN", "!kcab ruoy hctaw", "ypeerC",
