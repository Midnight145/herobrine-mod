...
public class HerobrineBuilder {
    ...
    private void placeBlock(World world, int x, int y, int z, int xOffset, int yOffset, int zOffset) {
        if (tunnel == 1) {
            if (blocks[xOffset][yOffset][zOffset] != Blocks.grass && blocks[xOffset][yOffset][zOffset] != Blocks.dirt
                && blocks[xOffset][yOffset][zOffset] != Blocks.cobblestone
                && blocks[xOffset][yOffset][zOffset] != Blocks.bedrock
                && blocks[xOffset][yOffset][zOffset] != Blocks.sandstone) {
                world.setBlock(
                    x + xOffset + 1,
                    y + yOffset,
                    z + zOffset + 1,
                    blocks[xOffset][yOffset][zOffset],
                    meta[xOffset][yOffset][zOffset],
                    3);
            }
        } else {
            world.setBlock(
    ...
    public void load(String file, int type) {
        ...
        try {
            ...
            if (type == 0) {
                ...
                location = HerobrineMod.class
                    .getResourceAsStream("/assets/herobrinemod/buildings/constructions/" + file);
            }
    ...
    private boolean areaIsEmpty(World world, int startX, int startY, int startZ) {
        ...
        if (tunnel == 0) {
                ...
                for (int y = 0; y < blocks[0].length; y++) {
                    for (int z = 0; z < blocks[0][0].length; z++) {
                        ...
                        if (world.getBlock(startX + x + 1, startY + y, startZ + z + 1) != Blocks.glass_pane
                            ...
                            counter++;
                            if (blocks[0][0].length * blocks[0].length / 2 < counter) {
                                return false;
                            }
                        }
                    }
            ...
            for (int x = 0; x < blocks.length; x++) {
                for (int z = 0; z < blocks[0][0].length; z++) {
                    if (world.getBlock(startX + x + 1, startY - 1, startZ + z) == Blocks.air) {
                        return false;
                    }
                ...
                for (int y = 0; y < blocks[0].length; y++) {
                    for (int z = 0; z < blocks[0][0].length; z++) {
                        if (world.getBlock(startX + x + 1, startY + depth + y, startZ + z + 1) == Blocks.sand
                            ...
                            || world.getBlock(startX + x + 1, startY + depth + y, startZ + z + 1)
