...
public class HerobrineBuilder {
			...
			for (int j = 0; j < id[0].length; j++) {
				for (int k = 0; k < id[0][0].length; k++) {
					if (!isNonSolid(id[i][j][k])) {
						...
						if (tunnel == 1) {
							if (id[i][j][k] != Block.getBlockById(2) && id[i][j][k] != Blocks.grass
									&& id[i][j][k] != Blocks.dirt && id[i][j][k] != Block.getBlockById(3)
									&& id[i][j][k] != Block.getBlockById(4) && id[i][j][k] != Block.getBlockById(7)
									&& id[i][j][k] != Block.getBlockById(24)) {
		...
		for (int i = 0; i < id.length; i++) {
			for (int j = 0; j < id[0].length; j++) {
				for (int k = 0; k < id[0][0].length; k++) {
					...
					if (id[i][j][k] == Block.getBlockById(27) || id[i][j][k] == Block.getBlockById(28)
							|| id[i][j][k] == Block.getBlockById(66)) {

					}
					if (j < id[0].length - 1
							&& (id[i][j][k] == Block.getBlockById(64) || id[i][j][k] == Block.getBlockById(71))) {
	...
	public void load(String file, int type) {
		...
		try {
			...
			if (type == 0) {
				loca = HerobrineMod.class.getResourceAsStream("/assets/herobrinemod/buildings/traps/" + file);
			}
			if (type == 1) {
				loca = HerobrineMod.class.getResourceAsStream("/assets/herobrinemod/buildings/constructions/" + file);
			}

			final BufferedReader in = new BufferedReader(new InputStreamReader(loca));
			...
			final Block ai[][][] = new Block[le1][le2][le3];
			final int ai1[][][] = new int[le1][le2][le3];
	...
	private static boolean areaIsEmpty(World world, int i, int j, int k) {
		...
		int counter = 0;
		final Block[][][] ai = blockID;
		if (tunnel == 0) {
			for (int l = 0; l < ai.length; l++) {
				for (int i1 = 0; i1 < ai[0].length; i1++) {
					for (int j1 = 0; j1 < ai[0][0].length; j1++) {

						if (world.getBlock(i + l + 1, j + i1, k + j1 + 1) != Blocks.glass_pane
								&& world.getBlock(i + l + 1, j + i1, k + j1 + 1) != Blocks.air
								&& world.getBlock(i + l + 1, j + i1, k + j1 + 1) != Block.getBlockById(232)
								&& world.getBlock(i + l + 1, j + i1, k + j1 + 1) != Blocks.tallgrass) {
							counter++;
							if (ai[0][0].length * ai[0].length / 2 < counter) { return false; }
						}
					}
				...
				for (int l2 = 0; l2 < ai.length; l2++) {
					for (int i2 = 0; i2 < ai[0][0].length; i2++) {
						...
						if (world.getBlock(i + l2 + 1, j - 1, k + i2) == Blocks.air) { return false; }
			...
			for (int l = 0; l < ai.length; l++) {
				for (int i1 = 0; i1 < ai[0].length; i1++) {
					for (int j1 = 0; j1 < ai[0][0].length; j1++) {
						if (world.getBlock(i + l + 1, j + depth + i1, k + j1 + 1) == Blocks.sand
								|| world.getBlock(i + l + 1, j + depth + i1, k + j1 + 1) == Blocks.air
								|| world.getBlock(i + l + 1, j + depth + i1, k + j1 + 1) == Block.getBlockById(232)) {
							counter++;
							if (5 < counter) { return false; }
						}
					}
	...
	public static Block[][][] getBlockID() { return blockID; }

	public static void setBlockID(Block ai[][][]) { blockID = ai; }

	public static int[][][] getBlockMeta() { return blockMeta; }

	public static void setBlockMeta(int ai[][][]) { blockMeta = ai; }
	...
	public static boolean isNonSolid(Block i) {
		final int id = Block.getIdFromBlock(i);
		return id == 70 || id == 72 || id == 46 || id == 6 || id == 26 || id == 27 || id == 28 || id == 30 || id == 31
				|| id == 32 || id == 37 || id == 38 || id == 39 || id == 40 || id == 50 || id == 51 || id == 55
				|| id == 59 || id == 63 || id == 64 || id == 65 || id == 66 || id == 68 || id == 69 || id == 71
				|| id == 75 || id == 76 || id == 77 || id == 78 || id == 81 || id == 83 || id == 90 || id == 93
				|| id == 94 || id == 96;
	}
